...
int x509parse_cert_info(char *buf, size_t size, const char *prefix,
    ...
    switch (crt->sig_alg) {
        case SIG_RSA_MD2    :
            ret = snprintf(p, n, "MD2");
            break;
        case SIG_RSA_MD4    :
            ret = snprintf(p, n, "MD4");
            break;
        case SIG_RSA_MD5    :
            ret = snprintf(p, n, "MD5");
            break;
        case SIG_RSA_SHA1   :
            ret = snprintf(p, n, "SHA1");
            break;
        case SIG_RSA_SHA224 :
            ret = snprintf(p, n, "SHA224");
            break;
        case SIG_RSA_SHA256 :
            ret = snprintf(p, n, "SHA256");
            break;
        case SIG_RSA_SHA384 :
            ret = snprintf(p, n, "SHA384");
            break;
        case SIG_RSA_SHA512 :
            ret = snprintf(p, n, "SHA512");
            break;
        default:
            ret = snprintf(p, n, "???");
            break;
...
static int x509parse_verify_child(
    ...
    *flags |= x509parse_verifycrl(child, parent, ca_crl);
    ...
    if (grandparent != NULL) {
         ...
         * Part of the chain
         */
        ret = x509parse_verify_child(parent, grandparent, trust_ca, ca_crl, path_cnt + 1, self_cnt,
                                     &parent_flags, f_vrfy, p_vrfy);
        if (ret != 0)
            return (ret);
    } else {
        ret = x509parse_verify_top(parent, trust_ca, ca_crl, path_cnt + 1, self_cnt, &parent_flags,
                                   f_vrfy, p_vrfy);
        if (ret != 0)
            return (ret);
    }
...
int x509parse_verify(x509_cert *crt,
    ...
    *flags = 0;
    ...
    if (parent != NULL) {
         ...
         * Part of the chain
         */
        ret = x509parse_verify_child(crt, parent, trust_ca, ca_crl, pathlen, selfsigned, flags,
                                     f_vrfy, p_vrfy);
        if (ret != 0)
            return (ret);
    } else {
        ret = x509parse_verify_top(crt, trust_ca, ca_crl, pathlen, selfsigned, flags, f_vrfy,
                                   p_vrfy);
        if (ret != 0)
            return (ret);
    }
