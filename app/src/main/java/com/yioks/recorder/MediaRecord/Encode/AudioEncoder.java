package com.yioks.recorder.MediaRecord.Encode;
...
public class AudioEncoder {
    ...
    public void startRecordAudioLoop() {
        ...
        if (handler != null)
            handler.sendMessage(handler.obtainMessage(
                    EncoderThread.EncoderHandler.MSG_FRAME_Start_Loop));
    }
    ...
    private static class EncoderThread extends Thread {
        ...
        //写入编码器
        private boolean writeToEncoder(ByteBuffer buf, AudioRecord recorder, boolean isRecord) throws Exception {
            if (isRecord) {
                int readSize = recorder.read(buf, bufferSizeInBytes);
                if (readSize > 0) {
         ...
         * 读取PCM并编码音频
         */
        private boolean readAndEncode(AudioRecord recorder, boolean isRecord) {
            if (audioRecord == null)
                return true;
            ByteBuffer buf = ByteBuffer.allocateDirect(bufferSizeInBytes);
            try {
                if (!writeToEncoder(buf, recorder, isRecord))
                    return true;
                readFromEncoder();
            } catch (Exception e) {
                e.printStackTrace();
         ...
         */
        boolean startVideoFrameLoop() {
            while (record) {
                synchronized (AudioEncoder.class) {
                    if (!readAndEncode(audioRecord, record)) {
                        if (record)
            ...
            //写入最后一帧
            readAndEncode(audioRecord, false);
            return true;
        ...
        private static class EncoderHandler extends Handler {
             ...
             */
            public EncoderHandler(AudioEncoder.EncoderThread et) {
            ...
            public void handleMessage(Message msg) {
                ...
                AudioEncoder.EncoderThread encoderThread = mWeakEncoderThread.get();
                ...
                switch (what) {
                    case MSG_FRAME_Start_Loop:
                        encoderThread.succeed = encoderThread.startVideoFrameLoop();
